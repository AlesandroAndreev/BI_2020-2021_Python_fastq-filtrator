...
def filter_by_length(input_length_min, read_header):
    local_flag_to_write = False
    if get_read_length(read_header) >= input_length_min:
...
def filter_by_gc_content(input_gc_maximum, input_gc_minimum, sequence):
    local_flag_to_write = False
    if input_gc_minimum - 1 < count_gc_content(sequence) < input_gc_maximum:
        local_flag_to_write = True
    return local_flag_to_write
